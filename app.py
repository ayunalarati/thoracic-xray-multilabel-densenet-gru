...
def get_preset_image(preset_id):
    ...
    return {"error": "Preset image not found"} # 404
...
async def predict(request: Request):
    ...
    try:
        ...
        req_data = {}
    threshold = float(req_data.get("threshold", THRESHOLD))
    ...
    if not img_bytes:
        ...
        if os.path.exists(default_file):
            ...
            return {"error": "No image provided and default image not found."} # 400
...
async def compute_scorecam(request: Request):
    """
...
def history():
    ...
    if not DB_ENABLED:
        return {"error": "Database not configured. Set SUPABASE_URL and SUPABASE_ANON_KEY in .env"} # 503
    try:
...
def db_health():
    ...
    if not DB_ENABLED:
        return {"status": "disabled", "message": "Supabase belum dikonfigurasi."})
    try:
        ok = db_health_check()
        return {"status": "online" if ok else "error", "supabase_connected": ok})
    except Exception as e:
        return {"status": "error", "message": str(e)} # 500



# Mount Gradio to satisfy Hugging Face Spaces free tier
def api_status():
    return "CITRA X-RAY FastAPI Backend is Active!"

demo = gr.Interface(fn=api_status, inputs=None, outputs="text", title="CITRA X-RAY API")
